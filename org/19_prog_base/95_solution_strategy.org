...
* 整数转罗马数字
  ...
  int bit = 0;
  while(val != 0) {
    temp = val % 10; // get low bit

    if (temp = 4) {
    ans = symbol[bit][0] + symbol[bit][1] + ans;

    } else if (temp = 9) {
    ans = symbol[bit][0] + symbol[bit+1][0] + ans;
    }
    else if (temp >=1 && temp <= 3){
    for(int i = 0; i< temp; i++){
    ans = symbol[bit][0] + ans;
    }
  ...
  为什么我第一时间想到的是 internal state?
  是受了题目描述的影响, 题目描述中就是使用了分支描述


  官方题解更优. 官方题解是循环减去罗马数字的最大值, 直到num 为0;
  而我的解题, 则是根据输入来使用不同的action ( 仔细想想, 确实只是输入, 还没有internal state)
  我的解题更接近turing machine. 而官方解体是对题目进一步思考之后的结果
